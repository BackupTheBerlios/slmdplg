...
public class EnjutoRolUltimoDefensa extends EnjutoRol {
	...
	public EnjutoRolUltimoDefensa(EnjutoMojamuTeam jugador, SocSmall robot){
	...
	public void actuarUltimoDefensa() {
		...
		if (jugador.oponentes.length == 5){
			...
			if((abstract_robot.getPosition(curr_time).x)*jugador.SIDE < 0) {
				...
				Vec2 cercaDeMiCampo = new Vec2(jugador.ourGoal.x + (-jugador.SIDE)*EnjutoMojamuTeam.LONGITUD_CAMPO/8, jugador.ourGoal.y);
