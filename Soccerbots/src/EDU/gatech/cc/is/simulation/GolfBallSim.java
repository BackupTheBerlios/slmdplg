...
public class GolfBallSim extends AttractorSim implements SimulatedObject,SocRef 
		...
		if (kickOffReq) {
            ...
            */
            ...
            if ((position.x > 1.37)&&(Math.abs(position.y)<0.25))
            	...
            	westscore++;
                kickoff();
            	System.out.println("isbc:goal:west");
            	}
            	...
            	eastscore++;
                kickoff();
            	System.out.println("isbc:goal:east");
            	}
            ...
            if (avg_dist < 0.001) {
                /* Added for special kickoff */
                if (Math.abs(position.t) != Vec2.PI)
                    kickoff();
            }
        }             
	}
     ...
     * line that separates both fields 
     */
    private void specialKickoff() {
        shotclock = TIMEOUT;
        Random randGen = new Random();
        position.setr(randGen.nextFloat());
        if (randGen.nextBoolean()) {
            position.sett(Vec2.PI/2);            
        }
        else 
            position.sett(-Vec2.PI/2);
        velocity.setr(0);
