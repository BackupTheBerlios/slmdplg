...
public class SimulationCanvas extends Canvas implements Runnable
	...
	private String westTeamName;
	private String eastTeamName;
    
    /**
     * The reference to the ball (for kickoff)
     */
    private GolfBallSim theBall;
	...
	private	void loadEnvironmentDescription() throws IOException
		...
		while (in.nextToken() != StreamTokenizer.TT_EOF)
			...
			if (in.ttype == StreamTokenizer.TT_WORD)
				...
				/*--- it is the west team name---*/
				//FORMAT: westTeam <STRING>
				if (token.equalsIgnoreCase("westname"))
					{
					if (in.nextToken()==
						StreamTokenizer.TT_NUMBER)
						{
						...
						{
							westTeamName = in.sval;
						
						}
					}
				
				/*--- it is the east team name---*/
				//FORMAT: eastTeam <STRING>
				if (token.equalsIgnoreCase("eastname"))
					{
					if (in.nextToken()==
						StreamTokenizer.TT_NUMBER)
						{
				...
				if (token.equalsIgnoreCase("object"))
                    ...
                    if (token.equals("EDU.gatech.cc.is.simulation.GolfBallNoiseSim")) {
                        // We have created the ball so we store the reference to the ball
                        theBall = (GolfBallSim) obj;
                    }
					}
	...
	*/
	public SimulationCanvas(Frame p, int w, int h,
		...
		{
		westTeamName = "westTeam";
		eastTeamName = "eastTeam";
		if (p == null) 
			{
			graphics_on = false;
			pause = false;
	...
	public void run()
		...
		while (keep_running)
            ...
            frameTime = current_time - startrun;
            while ((current_time-startrun)>timestep){
    			robots_done = true;
	...
	*/
	...
	public void start()
		...
		if (description_file_loaded)
			...
			startrun = System.currentTimeMillis();
			frames = 0;
			String windowTitle = westTeamName;
			windowTitle += " VS ";
			windowTitle += eastTeamName;
